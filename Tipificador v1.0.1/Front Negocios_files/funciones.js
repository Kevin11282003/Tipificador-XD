...
// * Funcion para tiempo en HH:MM
function call_time_hhmm(event) {
    const valor = event.target.value;
    const codigoTecla = event.which || event.keyCode;
  
    if (codigoTecla !== 58 && (codigoTecla < 48 || codigoTecla > 57)) {
      event.preventDefault();
    } else if (valor.length === 2 && codigoTecla !== 8) {
      event.target.value = valor + ':';
    }
}

// * Funcion para tiempo en HH:MM:SS
function call_time_hhmmss(event) {
    const valor = event.target.value;
    const codigoTecla = event.which || event.keyCode;

    // Verificar si se ingresó un número del 0 al 9 o dos puntos
    if (codigoTecla !== 58 && (codigoTecla < 48 || codigoTecla > 57)) {
        event.preventDefault();
    } else if (valor.length === 2 && codigoTecla !== 8) {
        event.target.value = valor + ':';
    } else if (valor.length === 5 && codigoTecla !== 8) {
        event.target.value = valor + ':';
...
// * Obtengo los campos y controlo los eventos
let elementosEdito = document.querySelectorAll('.required');
for (let i = 0; i < elementosEdito.length; i++) {
  elementosEdito[i].addEventListener('change', function() {
    ctrl_visual(this);
  });
  elementosEdito[i].addEventListener('focus', function() {
    ctrl_visual(this);
  });
  elementosEdito[i].addEventListener('keyup', function() {
    ctrl_visual(this);
  });
}
// * Ejecuto control visual
function ctrl_visual(elemento) {
    if (elemento.value === "" || elemento.value == "Elige una opción") {
      elemento.style.borderColor = "red";
      elemento.style.backgroundColor = "#FAF8F8";
    } else {
        elemento.style.borderColor = "#1e91d7";
